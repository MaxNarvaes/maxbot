...
function endGame(winner) { // handles the end of a game : no stopGame call inside
    ...
    if (winner == Team.RED) {
        ...
        room.sendAnnouncement("🏆 " + currentTeams[0].longName + " gana el partido! [" + scores.red + " - " + scores.blue + "]", null, 0x6ECAFF, 'bold');
    }
    else if (winner == Team.BLUE) {
        streak = 1;
        room.sendAnnouncement("🏆 " + currentTeams[1].longName + " gana el partido! [" + scores.blue + " - " + scores.red + "]", null, 0x6ECAFF, 'bold');
    }
    else {
        streak = 0;
    ...
    if (GKList.length > 1) {
        if (scores.red == 0 && GKList[1]) {
            room.sendAnnouncement("🧤 VALLA INVICTA PARA " + GKList[1].name + "! 🧤", null, 0xFFFFFF, 'normal');
        } else if (scores.blue == 0 && GKList[0]) {
            room.sendAnnouncement("🧤 VALLA INVICTA PARA " + GKList[0].name + "! 🧤", null, 0xFFFFFF, 'normal');
        }
    }
    ...
    if (topGoalScorer != undefined && topGoalScorer.name != undefined && scores.red + scores.blue > 0) {
        room.sendAnnouncement("🥇 " + topGoalScorer.name + " FUE EL GOLEADOR DEL PARTIDO CON " + topGoals + " GOLES! ⚽", null, 0xFFFFFF, 'normal');
    }

    updateStats();
    sendDiscordWebhook(scores);
    room.sendAnnouncement("🎥 ¡EL PARTIDO FUÉ GRABADO Y SE ENVIÓ A NUESTRO DISCORD!: " + `SINDICATOREPLAY-${getDate()}-${roomNumber + 1}.hbr2`, null, Colors.Azulclaro, 'bold');
...
function updateStats() {
    ...
    if (players.length >= 2) {
        ...
        room.sendAnnouncement("Estadisticas guardadas", null, Colors.Branco, 'italic');
    }
}
...
room.onGameStart = function (byPlayer) {
    if (!initializing) {
        ...
        allReds = [];
        allBlues = [];
        room.sendAnnouncement("「📌」Utiliza !gk para ser el arquero de el equipo!", null, 0xFFFFFF, "bold", 1);
        room.sendAnnouncement("「📌」¡Escribe la 't' al principio de tu mensaje para enviar un mensaje al equipo!", null, 0xFFFFFF, "bold", 1);
