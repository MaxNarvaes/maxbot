...
console.logg("adminPassword : " + adminPassword);
...
function chooseMap() {
    if (players.length < 5) {
        ...
        if (currentMap != v3Map) {
            //console.logg("chose x3");
            return v3Map;
        }
        return null;
    } else if (players.length <= 10) {
        if (currentMap != v4Map) {
            //console.logg("chose x4");
            return v4Map;
        }
        return null;
    } else if (players.length <= 12) {
        if (currentMap != v6Map) {
            //console.logg("chose x6");
            return v6Map;
        }
        return null;
    } else if (players.length <= 14) {
        if (currentMap != v7Map) {
            //console.logg("chose x7");
            return v7Map;
        }
        return null;
...
function customBalancePlayer(player) {
    if (teamR.length <= teamB.length) {
        ...
        room.setPlayerTeam(pid, Team.RED);
        teamR.push(player);
        //console.logg("+1 red on join")
    } else {
        var pid = player.id;
        room.setPlayerTeam(pid, Team.BLUE);
        teamB.push(player);
        //console.logg("+1 blue on join")
    }
}
...
room.onPlayerJoin = function (player) {
    ...
    //console.logg("entro " + player.name);
    updateTeams();
...
room.onPlayerChat = function (player, message) {
    ...
    //console.logg(adminTag + player.name + ": " + message);
